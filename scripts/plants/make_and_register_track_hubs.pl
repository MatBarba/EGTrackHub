...
GetOptions(
  ...
  "file_location_of_study_ids_or_species=s" =>
    \$file_location_of_study_ids_or_species,
...
my %study_ids_from_AE =
  %{ EGTH::ArrayExpress::get_completed_study_ids_for_plants(
    $plant_names_href_EG) };
...
if ($study_ids_file_content) {
  ...
  my %eg_species_names = %{ EGTH::EG::get_plant_names() };
  ...
  while ( my $line = readline $locations_fh ) {
    ...
    if ( $eg_species_names{$line} ) {
      ...
      die "\nPlant name "
        . $line
        . " is not part of the EnsemblGenomes plant names. Please run again the pipeline using these names inside the file $file_location_of_study_ids_or_species : \n\n"
  ...
  foreach my $species_name ( keys %species_names ) {
    ...
    my %study_ids_of_plant =
      %{ EGTH::ArrayExpress::get_study_ids_for_plant($species_name) };
...

{
  print_calling_params_logging(
    ...
    $server_dir_full_path, $server_url, $track_hub_visibility,
    $file_location_of_study_ids_or_species
  );
  ...
  if ( $plant_names_AE_response_href == 0 ) {
    ...
    die
      "Could not get plant names with processed runs from AE API calling script "
      . __FILE__
      . " line: "
      . __LINE__ . "\n";
  }
  ...
  my $unsuccessful_studies_href = make_register_THs_with_logging(
    $registry_obj,         \%study_ids,
    $server_dir_full_path, $organism_assmblAccession_EG_href,
  ...
  foreach my $reason_of_failure ( keys %$unsuccessful_studies_href ) { # hash looks like; $unsuccessful_studies{"Missing all Samples in AE REST API"}{$study_id}= 1;
    ...
    foreach my $failed_study_id (
      keys %{ $unsuccessful_studies_href->{$reason_of_failure} } )
    {
...
sub make_register_THs_with_logging {
  ...
  foreach my $study_id ( keys %$study_ids_href ) {
    ...
    if ( !$sample_ids_href ) { # there are cases where the AE API returns a study with the the sample_ids field to be null , I want to skip these studies
      $unsuccessful_studies{"Missing all Samples in AE REST API"}{$study_id} =
        1;
      next;
    }
    ...
    $line_counter++;
    print
      "$line_counter.\tcreating track hub in the server for study $study_id\t";
    ...
    if ( $script_output !~ /..Done/ ) { # if for some reason the track hub didn't manage to be made in the server, it shouldn't be registered in the Registry, for example Robert gives me a study id as completed that is not yet in ENA
        ...
        )
      {
        ...
        $unsuccessful_studies{"At least 1 cram file of study is not yet in ENA"}
          {$study_id} = 1;
...
sub print_calling_params_logging {
  ...
  my (
    ...
    $track_hub_visibility, $file_location_of_study_ids_or_species
  ) = @_;
  ...
  print "\n* Ran this pipeline:\n\n";
  print
    "perl make_and_register_track_hubs.pl -server_dir_full_path $server_dir_full_path -server_url $server_url -th_visibility $track_hub_visibility -file_location_of_study_ids_or_species $file_location_of_study_ids_or_species";
...
sub print_registry_registered_number_of_th {
  ...
  my $all_track_hubs_in_registry_href =
  ...
  print "There are in total "
    . scalar( keys %$all_track_hubs_in_registry_href );
...
sub register_track_hub_in_TH_registry {
  ...
  my $hub_assemblies = get_assemblies_for_study(
    $study_obj,
    $eg_assemblies
  );
  my $output = $registry->register_track_hub(
    $study_id,
    $hub_txt_url,
    $hub_assemblies
  );
...
sub get_assemblies_for_study {
  my $study_obj = shift;
  my $eg_assemblies = shift; #$hash{"brachypodium_distachyon"} = "GCA_000005505.1"
  ...
  foreach my $organism_name_AE ( keys %study_organism_names_AE_assembly_name ) {
    ...
    if ( $eg_assemblies->{$organism_name_AE} ) {

      my $string =
          $study_organism_names_AE_assembly_name{$organism_name_AE} . ","
        . $eg_assemblies->{$organism_name_AE};
