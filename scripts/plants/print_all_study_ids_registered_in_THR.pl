...
if ( not $registry_user_name or not $registry_pwd ) {
  die(
    "You need to export both THR_USER and THR_PWD (TrackHub registry username and password)\n"
  );
}

my $registry_obj =
  Registry->new( $registry_user_name, $registry_pwd, "public" ); # dosn't matter the visibility setting in this case
