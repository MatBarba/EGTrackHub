...
my %study_ids_from_AE = %{EGTH::ArrayExpress::get_completed_study_ids_for_plants($plant_names_href_EG)};
...
if ($study_ids_file_content){
  ...
  foreach my $species_name (keys %species_names){
    ...
    my %study_ids_of_plant = %{EGTH::ArrayExpress::get_study_ids_for_plant($species_name)};
