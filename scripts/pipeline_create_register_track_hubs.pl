...
defined $registry_user_name and $registry_pwd
  or die
  "Track Hub Registry username and password are required to be set as shell variables\n";
...
{                                                         # main method
  ...
  print_calling_params_logging(
    $registry_user_name, $registry_pwd,
    $server_dir_full_path, $server_url, $track_hub_visibility, $from_scratch
  );

  my $registry_obj = EGTH::Registry->new(
    $registry_user_name, $registry_pwd,
    $track_hub_visibility
  );
  ...
  my $plant_names_AE_response_href =
    EGTH::ArrayExpress::get_plant_names_AE_API();

  if ( $plant_names_AE_response_href == 0 ) {

    die
      "Could not get plant names with processed runs from AE API callin script "
      . __FILE__
      . " line: "
      . __LINE__ . "\n";
  }
  ...
  my $organism_assmblAccession_EG_href =
    EGTH::EG::get_species_name_assembly_id_hash(); #$hash{"brachypodium_distachyon"} = "GCA_000005505.1"         also:  $hash{"oryza_rufipogon"} = "0000"

  if ($from_scratch) {

    $unsuccessful_studies_href = run_pipeline_from_scratch_with_logging(
      $registry_obj,
      $study_ids_href_AE,                $server_dir_full_path,
      $organism_assmblAccession_EG_href, $plant_names_AE_response_href
    );
    ...
    $unsuccessful_studies_href =
      run_pipeline_with_incremental_update_with_logging(
      $study_ids_href_AE,
      $registry_obj, $server_dir_full_path, $plant_names_AE_response_href,
  ...
  my (
    $study_id_biorep_ids_AE_currently_href,
    $plant_study_id_AE_currently_number_of_bioreps_href
    )
    = give_hashes_with_AE_current_stats(
    $study_ids_href_AE,
    $plant_names_AE_response_href
    );

  print_current_AE_studies_stats(
    $study_id_biorep_ids_AE_currently_href,
    $plant_study_id_AE_currently_number_of_bioreps_href,
    $server_dir_full_path
  );

  print_registered_TH_in_THR_stats_after_pipeline_is_run(
    $registry_obj,
    $plant_names_AE_response_href, $unsuccessful_studies_href
  );
  print_run_duration_so_far($start_time);
...
sub print_registered_TH_in_THR_stats {
  ...
  my $all_track_hubs_in_registry_after_update_href =
  ...
  foreach my $hub_name ( keys %{$all_track_hubs_in_registry_after_update_href} )
  {
    ...
    my %bioreps_hash =
      %{ $registry_obj->give_all_bioreps_of_study_from_Registry($hub_name) };
    map { $distinct_bioreps{$_}++ } keys %bioreps_hash;
  }

  print "There are in total "
    . scalar( keys %{$all_track_hubs_in_registry_after_update_href} );
  print " track hubs with total "
    . scalar( keys %distinct_bioreps )
    . " bioreps registered in the Track Hub Registry\n\n";
...
sub run_pipeline_with_incremental_update_with_logging {
  ...
  my $registered_track_hubs_href =
    $registry_obj->give_all_Registered_track_hub_names; # track hubs that are already registered

  remove_obsolete_studies(
    $study_ids_href_AE,          $registry_obj,
    $registered_track_hubs_href, $server_dir_full_path
  ); # if there are any obsolete track hubs, they are removed from the THR and the server

  my ( $new_study_ids_aref, $common_study_ids_aref ) =
    get_new_and_common_study_ids(
    $study_ids_href_AE,
    $registered_track_hubs_href
    );
  ...
  if ( scalar(@$new_study_ids_aref) == 0 ) {
    print
      "No new studies are found between current AE API studies and registered studies in the THR\n";
  }
  else {
    print "\nNew studies ("
      . scalar(@$new_study_ids_aref)
      . " studies) from last time the pipeline was run:\n\n";
  }

  %unsuccessful_studies = create_new_studies_in_incremental_update(
    $new_study_ids_aref,
    $server_dir_full_path, $plant_names_AE_response_href, $registry_obj,
    $organism_assmblAccession_EG_href, \%unsuccessful_studies
  );

  %unsuccessful_studies = update_common_studies(
    $common_study_ids_aref,            $registry_obj,
    $plant_names_AE_response_href,     $server_dir_full_path,
    $organism_assmblAccession_EG_href, \%unsuccessful_studies
  );
...
sub update_common_studies {
  ...
  my %common_studies_to_be_updated = %{
    get_study_ids_to_be_updated(
      $common_study_ids_aref, $registry_obj, $plant_names_AE_response_href
    )
  }; #hash looks like: $common_study_ids_to_be_updated {$common_study_id}=\@holder_of_reason_of_update;
  ...
  if ( scalar( keys %common_studies_to_be_updated ) == 0 ) {
    ...
    print "\nStudies to be updated ("
      . scalar( keys %common_studies_to_be_updated )
      . " studies) from last time the pipeline was run:\n";
  }
  print "\n\n";
  ...
  foreach my $study_id ( keys %common_studies_to_be_updated ) {
    ...
    my $study_obj =
      EGTH::AEStudy->new( $study_id, $plant_names_AE_response_href );
    ...
    my $date_registry_last =
      localtime( $registry_obj->get_Registry_hub_last_update($study_id) )
      ->strftime('%F %T');
    my $reason_of_unsuccessful_study;
    ( $reason_of_unsuccessful_study, $study_counter ) =
      make_and_register_track_hub(
      $study_obj,                        $registry_obj,
      $old_study_counter,                $server_dir_full_path,
      $organism_assmblAccession_EG_href, $plant_names_AE_response_href
      );
    if ( $reason_of_unsuccessful_study ne "successful_study" ) {
      $unsuccessful_studies_href->{$reason_of_unsuccessful_study}{$study_id} =
        1;
    }
...
#######
    ...
    if ( $info_array[0] eq "diff_time_only" ) {
      ...
      my $date_cram_created =
        localtime( $study_obj->get_AE_last_processed_unix_date )
        ->strftime('%F %T');

      print " Last registered date: "
        . $date_registry_last
        . ", Max last processed date of CRAMS from study: "
        . $date_cram_created . "\n";
      ...
      print " Last registered date: "
        ...
        . ", Max last processed date of CRAMS from study: "
        . $date_cram_created
        . " and also different number/ids of runs: "
        . " Last Registered number of runs: "
        . $info_array[1]
        . ", Runs in Array Express currently: "
        . $info_array[2] . "\n";

    }
    elsif ( $info_array[0] eq "diff_bioreps_only" ) {

      print " Different number/ids of runs: Last Registered number of runs: "
        . $info_array[1]
        . ", Runs in Array Express currently: "
        . $info_array[2] . "\n";
      ...
      print "Don't know why this study is being updated\n"
        and print STDERR
        "Something went wrong with common study between the THR and AE $study_id that I decided to update; don't know why needs updating\n";
    }
  }
...
sub create_new_studies_in_incremental_update {
  ...
  foreach my $study_id (@$new_study_ids_aref) {
    ...
    ( $return_reason_of_unsuccessful_study, $study_counter ) =
      make_and_register_track_hub(
      $study_obj,                        $registry_obj,
      $old_study_counter,                $server_dir_full_path,
      $organism_assmblAccession_EG_href, $plant_names_AE_response_href
    ...
    if ( $return_reason_of_unsuccessful_study ne "successful_study" ) {
      $unsuccessful_studies_href->{$return_reason_of_unsuccessful_study}
        {$study_id} = 1;
    }
...
sub get_study_ids_to_be_updated { # gets a list of common study ids and decides which ones have changed, hence need updating
  ...
  foreach my $common_study_id (@$common_study_ids_array_ref) {
    ...
    my $study_obj =
      EGTH::AEStudy->new( $common_study_id, $plant_names_AE_response_href );

    my $AE_last_processed_unix_time =
      $study_obj->get_AE_last_processed_unix_date; # AE current response: the unix date of the creation the cram of the study (gives me the max date of all bioreps of the study)
    my $registry_study_created_date_unix_time =
      eval { $registry_obj->get_Registry_hub_last_update($common_study_id) }; # date of registration of the study
    ...
    if ($@) { # if the get_Registry_hub_last_update method fails to return the date of the track hub , then i re-do it anyways to be on the safe side
      ...
      $holder_of_reason_of_update[2] =
        scalar( keys %bioreps_in_Array_Express ); # in cell 2 of this table it's stored the number of bioreps of the common study in current Array Express API call

      my $are_bioreps_the_same = hash_keys_are_equal(
        \%bioreps_in_Registry,
        \%bioreps_in_Array_Express
      );    # returns 0 id they are not equal, 1 if they are

      if ( $registry_study_created_date_unix_time < $AE_last_processed_unix_time
        or $are_bioreps_the_same == 0 )
      { # if the study was registered before AE changed it OR it has now different bioreps, it needs to be updated

        if (
          $registry_study_created_date_unix_time < $AE_last_processed_unix_time
          and $are_bioreps_the_same == 1 )
        { # study was changed by AE after it was created and registered in the THR
          $holder_of_reason_of_update[0] = "diff_time_only";
          $common_study_ids_to_be_updated{$common_study_id} =
            \@holder_of_reason_of_update;
        }
        if ( $registry_study_created_date_unix_time >=
          $AE_last_processed_unix_time and $are_bioreps_the_same == 0 )
        {    # different number of bioreps
      ...
      die
        "I have to really die here since I don't know what happened in script "
        . __FILE__
        . " line "
        . __LINE__ . "\n";
    }
...
sub print_current_AE_studies_stats {
  ...
  foreach my $study_id ( keys %{$study_id_biorep_ids_AE_currently_href} ) {
    foreach my $biorep_id (
      keys %{ $study_id_biorep_ids_AE_currently_href->{$study_id} } )
    {
  ...
  print "\nThere are "
    . scalar( keys %total_bioreps )
    . " plant bioreps completed to date ( $current_date )\n";
  print "\nThere are "
    . scalar( keys %{$study_id_biorep_ids_AE_currently_href} )
    . " plant studies completed to date ( $current_date )\n";
  ...
  foreach
    my $plant ( keys %{$plant_study_id_AE_currently_number_of_bioreps_href} )
  {

    my $number_of_bioreps = 0;
    foreach my $study_id (
      keys %{ $plant_study_id_AE_currently_number_of_bioreps_href->{$plant} } )
    {

      $number_of_bioreps =
        $number_of_bioreps +
        $plant_study_id_AE_currently_number_of_bioreps_href->{$plant}
        {$study_id};
    }
  ...
  print "In total there are "
    . scalar( keys %{$plant_study_id_AE_currently_number_of_bioreps_href} )
    . " Ensembl plants done to date.\n\n";
  print
    "####################################################################################\n\n";
  ...
  print "There are in total "
    . give_number_of_dirs_in_ftp($server_dir_full_path)
    . " files in the ftp server\n\n";
}
...
sub print_registered_TH_in_THR_stats_after_pipeline_is_run {
  ...
  print_registered_TH_in_THR_stats(
    $registry_obj,
    $plant_names_AE_response_href
  );
  ...
  if ( scalar keys %$unsuccessful_studies_href > 0 ) {
    ...
    foreach my $reason ( keys %$unsuccessful_studies_href ) {
      foreach my $study_id ( keys %{ $unsuccessful_studies_href->{$reason} } ) {
        ...
        print $count_skipped_studies. ". "
          . $study_id . " ("
          . scalar( keys %bioreps_hash )
          . " bioreps)\t$reason\n";
      }
    }
  }
...
sub delete_registered_th_and_remove_th_from_server { # called only when option is run pipeline from scratch
  ...
  my $number_of_registered_th_initially =
...
sub run_pipeline_from_scratch_with_logging {
  ...
  delete_registered_th_and_remove_th_from_server(
    $registry_obj,
    $server_dir_full_path
  );
...
sub give_hashes_with_AE_current_stats {
  ...
  foreach my $study_id ( keys %{$study_ids_href_AE} ) {
    ...
    my $species_of_study_href =
      $study_obj->get_organism_names_assembly_names(); # hash-> key: organism_name , value: assembly_name , a study can have more than 1 species

    foreach my $species_name ( keys %{$species_of_study_href} ) {

      $plant_study_id_AE_currently_number_of_bioreps{$species_name}{$study_id}
        = scalar(
        keys %{ $study_obj->get_biorep_ids_by_organism($species_name) } );

    }
  }

  return (
    \%study_id_biorep_ids_AE_currently,
    \%plant_study_id_AE_currently_number_of_bioreps
  );
}
...
sub print_calling_params_logging {
  ...
  my (
    $registry_user_name,   $registry_pwd, $server_dir_full_path, $server_url,
    $track_hub_visibility, $from_scratch
  ) = @_;
  ...
  print "\n* Ran this pipeline:\n\n";
  print
    "perl pipeline_create_register_track_hubs.pl -server_dir_full_path $server_dir_full_path -server_url $server_url -th_visibility $track_hub_visibility";
  if ($from_scratch) {
  ...
  print
    "* I am using this Registry account:\n\n user:$registry_user_name \n password:$registry_pwd\n\n";
...
sub print_registry_registered_number_of_th {
  ...
  foreach my $hub_name ( keys %studies_last_run_of_pipeline ) {
    ...
    map { $distinct_bioreps_before_running_pipeline{$_}++ }
      keys %{ $registry_obj->give_all_bioreps_of_study_from_Registry($hub_name)
      };
  }

  print "\n* Before starting running the updates, there were in total "
    . scalar( keys %studies_last_run_of_pipeline )
    . " track hubs with total "
    . scalar( keys %distinct_bioreps_before_running_pipeline )
    . " bioreps registered in the Track Hub Registry under this account.\n\n";
...
sub get_list_of_all_AE_plant_studies_currently {
  ...
  my $study_ids_href = EGTH::ArrayExpress::get_completed_study_ids_for_plants(
    $plant_names_href_EG);
...
sub make_and_register_track_hub {
  ...
  my $track_hub_creator_obj =
    EGTH::TrackHubCreation->new( $study_id, $server_dir_full_path );
  my $script_output =
    $track_hub_creator_obj->make_track_hub($plant_names_AE_response_href);
  ...
  if ( $script_output !~ /..Done/ ) { # if for some reason the track hub didn't manage to be made in the server, it shouldn't be registered in the Registry, for example Robert gives me a study id as completed that is not yet in ENA
    ...
    print STDERR
      "Track hub of $study_id could not be made in the server - Folder $study_id will be deleted\n\n";
    ...
    if ( $script_output =~ /No ENA Warehouse metadata found/ ) {
      ...
      /Skipping this TH because at least 1 of the cram files of the TH is not yet in ENA/
      )
    {

      $return_reason_of_unsuccessful_study =
        "At least 1 cram file of study is not yet in ENA";
    ...
    my $output = register_track_hub_in_TH_registry(
      $registry_obj, $study_obj,
      $organism_assmblAccession_EG_href
    );
    ...
    if ( $output !~ /is Registered/ ) { # if something went wrong with the registration, i will not make a track hub out of this study
      ...
      $return_string = $return_string
        . "\t..Something went wrong with the Registration process -- this study will be skipped..\n";
...
sub get_assembly_names_assembly_ids_string_for_study {
  ...
  my %study_organism_names_AE_assembly_name =
    %{ $study_obj->get_organism_names_assembly_names }; # from AE API: $hash{"selaginella_moellendorffii"}= "TAIR10"

  foreach my $organism_name_AE ( keys %study_organism_names_AE_assembly_name )
  { # the organism name AE is the reference species name, so the same as the EG names

    if ( $organism_assmblAccession_EG_href->{$organism_name_AE} ) {

      my $string =
          $study_organism_names_AE_assembly_name{$organism_name_AE} . ","
        . $organism_assmblAccession_EG_href->{$organism_name_AE};
      push( @assembly_name_assembly_id_pairs, $string );

    }
    else {
      die
        "Could not find AE organism name $organism_name_AE in EG REST response\n";
    }
    $assemblyNames_assemblyAccesions_string =
      join( ",", @assembly_name_assembly_id_pairs );
  }
...
sub register_track_hub_in_TH_registry {
  ...
  my $assemblyNames_assemblyAccesions_string =
    get_assembly_names_assembly_ids_string_for_study(
    $study_obj,
    $organism_assmblAccession_EG_href
    );
  my $output = $registry_obj->register_track_hub(
    $study_id, $hub_txt_url,
    $assemblyNames_assemblyAccesions_string
  );
  return $output;
