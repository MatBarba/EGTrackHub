...
sub get_all_sample_keys {
  ...
  my $url =
    "http://www.ebi.ac.uk/ena/data/warehouse/usage?request=fields&result=sample";
...
sub get_hash_of_locations_of_cram_submissions {
  ...
  my $url =
    "http://www.ebi.ac.uk/ena/data/warehouse/filereport?accession=ERP014374&result=analysis&fields=first_public,submitted_ftp&download=txt";
  ...
  if ( $response->code != 200 or $response_string =~ /^ *$/ ) {
    ...
    if ( $flag_success == 0 or $response_string =~ /^ *$/ ) { # if after the 10 attempts I still don't get the metadata..
      ...
      print STDERR "Didn't get response for CRAM locations using url $url"
        . "\t"
        . $response->code . "\n\n";
      return 0;
    }
    ...
    foreach my $line (@lines) {
      ...
      my $cram_url  = $words[1];
      $cram_url = $url_protocol . '://' . $cram_url
        if $url_protocol and not $cram_url =~ /^(http|ftp):\/\//;
      ...
      if ( $cram_url =~ /.+\/(.+)\.cram/ ) {
        ...
        print STDERR
          "In method get_hash_of_locations_of_cram_submissions, module ENA.pm , line "
          . __LINE__
          . " could not get the cram name here $line in the regex\n";
      }
    }
...
sub get_ENA_cram_location {
  ...
  if ( not defined $unique_cram_locations_href ) {
    ...
    $unique_cram_locations_href =
      get_last_updated_cram_file_location_hash($all_cram_locations_href);
  }
