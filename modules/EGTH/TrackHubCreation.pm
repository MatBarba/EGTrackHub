...
sub new {
  ...
  defined $study_id and defined $server_dir_full_path
    or die
    "Object must be constructed using 2 parameters: study id and folder path\n";
...
sub make_track_hub { # main method, creates the track hub of a study in the folder/server specified
  ...
  foreach my $assembly_name ( keys %assembly_names ) {
    ...
    my $return_of_make_trackDbtxt_file = $self->make_trackDbtxt_file(
      $server_dir_full_path, $study_obj,
      $assembly_name
    );

    if ( !$return_of_make_trackDbtxt_file ) { # method returns 0 when there is no ENA warehouse metadata
      return
        ".. No ENA Warehouse metadata found for at least 1 of the sample ids\n";
    }
    if ( $return_of_make_trackDbtxt_file eq
      "at least 1 cram file of the TH that was not found in ENA" )
    {
      return
...
sub make_hubtxt_file {
  ...
  if ( $ena_study_title eq "Study title was not found in ENA" ) {
    ...
    $long_label =
        "longLabel <a href=\"http://www.ebi.ac.uk/ena/data/view/"
      . $study_id . "\">"
      . $study_id . "</a>" . "\n";
    ...
    $long_label =
      "longLabel $ena_study_title ; <a href=\"http://www.ebi.ac.uk/ena/data/view/"
      . $study_id . "\">"
      . $study_id . "</a>" . "\n";
  }
  print $fh $long_label;
...
sub make_trackDbtxt_file {
  ...
  my $study_id = $study_obj->id;
  my $trackDb_txt_file =
    "$ftp_dir_full_path/$study_id/$assembly_name/trackDb.txt";
  ...
  foreach my $sample_id (@sample_ids) {
    ...
    foreach my $biorep_id (
      keys %{ $study_obj->get_biorep_ids_from_sample_id($sample_id) } )
    {
      ...
      my $track_obj = $self->make_biosample_sub_track_obj(
        $study_obj,
        $biorep_id,
        $sample_id,
        $visibility
