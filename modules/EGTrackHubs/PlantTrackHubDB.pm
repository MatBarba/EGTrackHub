package EGTrackHubs::PlantTrackHubDB;
...
use Moose;
extends 'EGTrackHubs::TrackHubDB';
...
sub load_plant_data {
  ...
  my $ae_study = EGTrackHubs::AEStudy->new($self->id, $plant_names);
...
sub prepare_hub {
  ...
  if ($ena_study_title eq "Study title was not found in ENA") { 
    ...
    $self->longLabel("<a href=\"http://www.ebi.ac.uk/ena/data/view/$study_id\">$study_id</a>");
  } else {
    $self->longLabel("$ena_study_title; <a href=\"http://www.ebi.ac.uk/ena/data/view/$study_id\">$study_id</a>");
  }
  return 1;
}
...
sub prepare_genomes {
  ...
  foreach my $assembly_name (keys %{$assembly_names_href}) {
    ...
    my $genome = EGTrackHubs::TrackHubDB::Genome->new(
      id      => $assembly_name,
    ...
    foreach my $sample_id (@sample_ids) {
      ...
      foreach my $biorep_id (keys %$bioreps) {
        ...
        # Create a subtrack
        my $sub_track = $self->make_biosample_sub_track_obj($study, $biorep_id, $sample_id);
