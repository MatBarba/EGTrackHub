...
isa_ok(
  $trackHubCreator_obj, 'EGTH::TrackHubCreation',
  'the object constructed is of my class type'
);
dies_ok(
  sub { EGTH::TrackHubCreation->new($study_dir) },
  'Construction from wrong object should die'
);
...
my $return = $trackHubCreator_obj->make_trackDbtxt_file(
  $tmpdir, $study_obj,
  $assembly_id
);
file_exists_ok( $trackdb_file, "The file trackDb.txt was created" );
file_contains_like(
  $trackdb_file, $trackdb_format,
...

{
  ...
  is(
    $new_label, "electra_tapanari",
    'replace string 1 underscore by 1 space in string'
  );

  $new_label = EGTH::TrackHubCreation::printlabel_key("electra_tapanari");
  is(
    $new_label, "electra_tapanari",
    'no change in string with underscore and no space'
  );

  $new_label = EGTH::TrackHubCreation::printlabel_key("electra");
  is(
    $new_label, "electra",
    'no change in string with no underscore or space'
  );

  $new_label =
    EGTH::TrackHubCreation::printlabel_key("electra tapanari of angelos");
  is(
    $new_label, "electra_tapanari_of_angelos",
    'replace all spaces with underscores in string'
  );
}
...
# -----
{
  ...
  $new_label =
    EGTH::TrackHubCreation::printlabel_value("electra tapanari of angelos");
  is(
    $new_label, "\"electra tapanari of angelos\"",
    "put quotes to string with several spaces"
  );
}
...
my $expected_title =
  "Illumina Genome Analyzer IIx sequencing; Illumina sequencing of cDNAs generated from mRNAs_retro_PAAF";

my $ena_biorep_title =
  EGTH::TrackHubCreation::get_ENA_biorep_title( $study_obj, $biorep_id );
is(
  $ena_biorep_title, $expected_title,
  "ENA biorep title as expected currently"
);
...
my $expected_long_label =
  "Total mRNAs from callus, leaf, panicle before flowering, panicle after flowering, root, seed, and shoot of rice (Oryza sativa ssp. Japonica cv. Nipponbare) ; <a href=\"http://www.ebi.ac.uk/ena/data/view/SAMD00008650\">SAMD00008650</a>";
my $expected_metadata_pattern =
  qr/".+BST" biosample_id=SAMD00008650 germline=N description=\"Total mRNAs from callus, leaf, panicle before flowering, panicle after flowering, root, seed, and shoot of rice \(Oryza sativa ssp. Japonica cv. Nipponbare\)\" accession=SAMD00008650 environmental_sample=N scientific_name=\"Oryza sativa Japonica Group\" sample_alias=SAMD00008650 tax_id=39947 center_name=BIOSAMPLE secondary_sample_accession=DRS000668 first_public=2011-12-21/;

my $super_track_obj =
  $trackHubCreator_obj->make_biosample_super_track_obj($sample_id);
is( $super_track_obj->track, $sample_id, "super track name is as expected" );
is(
  $super_track_obj->longLabel, $expected_long_label,
  "super track long label is as expected"
);
...
# -----
my $expected_data_url =
  "http://ftp.sra.ebi.ac.uk/vol1/ERZ310/ERZ310303/E-MTAB-2037.biorep4.cram";
my $expected_track_long_label =
  "Illumina Genome Analyzer IIx sequencing; Illumina sequencing of cDNAs generated from mRNAs_retro_PAAF;<a href=\"http://www.ebi.ac.uk/arrayexpress/experiments/E-GEOD-55482/samples/?full=truehttp://www.ebi.ac.uk/~rpetry/bbrswcapital/E-MTAB-2037.bioreps.txt\">E-MTAB-2037.biorep4</a>";
...
is(
  $sub_track_obj->parent, $sample_id,
  "sub track parent name is as expected"
);
is(
  $sub_track_obj->bigDataUrl, $expected_data_url,
  "sub track cram url location is as expected"
);
is(
  $sub_track_obj->shortLabel, $expected_short_label,
  "sub track short label is as expected"
);
is(
  $sub_track_obj->longLabel, $expected_track_long_label,
  "sub track long label is as expected"
);
is(
  $sub_track_obj->type, $expected_file_type,
  "sub track file type is as expected"
);
is(
  $sub_track_obj->visibility, $expected_visibility,
  "sub track visibility is as expected"
);
